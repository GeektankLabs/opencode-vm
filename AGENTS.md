...
**Skills subsystem (single control surface):** `opencode-vm skills {status|on|off|list}` is the entire API. v0.4.2 ships one built-in and three opt-in packages. **Built-in (always active):** `webimg` (web image optimization pipeline — pure CLI tools pre-installed in base VM, no MCP, no credentials). **Opt-in:** `ecc-auto` (language-filtered ECC skills, ~30), `ecc-all` (every ECC skill, ~180), and `proxmox` (bundled Proxmox VE knowledge skill + MCP server). `ecc-auto` ↔ `ecc-all` are mutually exclusive. `proxmox` is independent of ECC; its `on` prompts interactively for API host/token (saved mode 0600 to `~/.opencode-vm/proxmox.env`), clones `canvrno/ProxmoxMCP` host-side for visibility, and triggers a one-time in-base-VM venv install at next `start`. `skills off proxmox` wipes `proxmox.env`. `webimg` is always mounted regardless of `SKILLS_PACKAGES` — its CLI tools (ImageMagick, libvips, cwebp, avifenc, pngquant, optipng, jpegoptim, gifsicle, svgo, potrace, exiftool) are part of the base VM provisioning. State lives in `~/.opencode-vm/skills.env` (field: `SKILLS_PACKAGES`). Active packages are resolved at `start_session` and rsynced into `$sess_share/config/opencode/skills/<pkg>/<skill>/`; a `skills-manifest.txt` lists what mounted. Per-package source roots let `proxmox` and `webimg` draw from bundled `skills/` directories (or sparse clones of this repo when running from an installed single-file copy).
